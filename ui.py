...
#-------------------- Class functions ----------------------------
    ...
    def drawSquares(self):
        ...
        for col in range(8):
            for row in range(8):
                ...
                pygame.draw.rect(self.screen, color, (x, y, self.squareLength, self.squareLength))
    ...
    def drawPieces(self, board):
        ...
        for (row, col), piece in board.grid.items():
            ...
            pygame.draw.circle(self.screen, color, (centerX, centerY), int(self.squareLength * 0.4))

            text = self.font.render(piece.label, True,
    ...
    def drawTurnIndicator(self, currentTurn):
        ...
        badge_color = colors.WHITE_PIECE if currentTurn == "white" else colors.BLACK_PIECE
        text = self.font.render(f"Turn: {currentTurn.capitalize()}", True, colors.TURN_TEXT)
    ...
    def drawHighlight(self, selectedSquare):
        ...
        x = self.boardStart + col *self.squareLength
        y = self.boardStart + row * self.squareLength
        pygame.draw.rect(self.screen, colors.SELECTED_HIGHLIGHT, (x, y, self.squareLength, self.squareLength), 3)


    def isInsideBoard(self, mouseX, mouseY):
        """ isInsideBoard will determine if the user click was inside the board """ 
        return (
            self.boardStart <= mouseX < self.boardStart + self.boardSize and
            self.boardStart <= mouseY < self.boardStart + self.boardSize
        )
    ...
    def isInsidePromotion(self, mouseX, mouseY):
        ...
        return (
            self.promoStart <= mouseX < self.promoStart + self.promoSize * 2 and 
            self.promoStart <= mouseY < self.promoStart + self.promoSize * 2
        )
        pass
    ...
    def drawPromotion(self, color):
        ...
        outerRect = pygame.Rect(self.promoStart - 50, self.promoStart - 50, self.promoSize + 180, self.promoSize + 180)
        ...
        pygame.draw.line(self.screen, tileColor, (separator_x, self.promoStart), (separator_x, self.promoStart + 2 * self.promoSize), 2)
        pygame.draw.line(self.screen, tileColor, (self.promoStart, separator_y), (self.promoStart + 2 * self.promoSize, separator_y), 2)